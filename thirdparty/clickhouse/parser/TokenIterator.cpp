...
UnmatchedParentheses checkUnmatchedParentheses(TokenIterator begin) {
  ...
  /// We have to iterate through all tokens until the end to avoid false
  /// positive "Unmatched parentheses" error when parser failed in the middle of
  /// the query.
  for (TokenIterator it = begin; it.isValid(); ++it) {
    if (it->type == TokenType::OpeningRoundBracket ||
        it->type == TokenType::OpeningSquareBracket) {
      stack.push_back(*it);
    } else if (it->type == TokenType::ClosingRoundBracket ||
      ...
      if ((stack.back().type == TokenType::OpeningRoundBracket &&
