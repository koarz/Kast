...
#include "clickhouse/base/find_symbols.h"
...
template <char quote>
Token quotedString(const char *&pos, const char *const token_begin,
                   const char *const end, TokenType success_token,
...
Token quotedStringWithUnicodeQuotes(
    const char *&pos, const char *const token_begin, const char *const end,
...
Token quotedHexOrBinString(const char *&pos, const char *const token_begin,
...
Token Lexer::nextTokenImpl() {
  ...
  auto comment_until_end_of_line = [&]() mutable {
    pos = find_first_symbols<'\n'>(
        pos, end); /// This means that newline in single-line comment cannot be
                   /// escaped.
  ...
  case '8':
  case '9': {
    /// The task is not to parse a number or check correctness, but only to skip
    /// it.

    /// Disambiguation: if previous token was dot, then we could parse only
    /// simple integer,
    ...
    if (prev_significant_token_type == TokenType::Dot) {
      ...
      while (pos < end && (isNumericASCII(*pos) ||
                           isNumberSeparator(false, false, pos, end)))
        ++pos;
    } else {
      ...
      bool hex = false;
      if (pos + 2 < end && *pos == '0' &&
          (pos[1] == 'x' || pos[1] == 'b' || pos[1] == 'X' || pos[1] == 'B')) {
        bool is_valid = false;
        if (pos[1] == 'x' || pos[1] == 'X') {
          if (isHexDigit(pos[2])) {
            hex = true;
      ...
      while (pos < end && ((hex ? isHexDigit(*pos) : isNumericASCII(*pos)) ||
                           isNumberSeparator(start_of_block, hex, pos, end))) {
        ++pos;
      ...
      if (pos < end && *pos == '.') {
        ...
        ++pos;
        while (pos < end &&
               ((hex ? isHexDigit(*pos) : isNumericASCII(*pos)) ||
                isNumberSeparator(start_of_block, hex, pos, end))) {
          ++pos;
          start_of_block = false;
        }
      ...
      if (pos + 1 < end &&
        ...
        while (pos < end &&
               (isNumericASCII(*pos) ||
                isNumberSeparator(start_of_block, false, pos, end))) {
          ++pos;
          start_of_block = false;
        }
      }
  ...
  case '\'':
    return quotedString<'\''>(pos, token_begin, end, TokenType::StringLiteral,
                              TokenType::ErrorSingleQuoteIsNotClosed);
  case '"':
    return quotedString<'"'>(pos, token_begin, end, TokenType::QuotedIdentifier,
                             TokenType::ErrorDoubleQuoteIsNotClosed);
  case '`':
    return quotedString<'`'>(pos, token_begin, end, TokenType::QuotedIdentifier,
                             TokenType::ErrorBackQuoteIsNotClosed);

  case '(': return Token(TokenType::OpeningRoundBracket, token_begin, ++pos);
  case ')': return Token(TokenType::ClosingRoundBracket, token_begin, ++pos);
  case '[': return Token(TokenType::OpeningSquareBracket, token_begin, ++pos);
  case ']': return Token(TokenType::ClosingSquareBracket, token_begin, ++pos);
  case '{': return Token(TokenType::OpeningCurlyBrace, token_begin, ++pos);
  case '}': return Token(TokenType::ClosingCurlyBrace, token_begin, ++pos);
  case ',': return Token(TokenType::Comma, token_begin, ++pos);
  case ';': return Token(TokenType::Semicolon, token_begin, ++pos);

  case '.': /// qualifier, tuple access operator or start of floating point
            /// number
  {
    /// Just after identifier or complex expression or number (for chained tuple
    /// access like x.1.1 to work properly).
    if (pos > begin &&
  ...
  case '+': return Token(TokenType::Plus, token_begin, ++pos);
  ...
  case '*': ++pos; return Token(TokenType::Asterisk, token_begin, pos);
  case '/': /// division (/) or start of comment (//, /*)
  {
    ...
    if (pos < end && (*pos == '/' || *pos == '*')) {
      ...
      pos = end;
      return Token(TokenType::ErrorMultilineCommentIsNotClosed, token_begin,
                   pos);
    }
  ...
  {         /// PostgreSQL has some operators using '#' character.
    /// For less ambiguity, we will recognize a comment only if # is followed by
    /// whitespace. or #! as a special case for "shebang". #hello - not a
    /// comment # hello - a comment
  ...
  case '%': return Token(TokenType::Percent, token_begin, ++pos);
  case '=': /// =, ==
  {
    ++pos;
    if (pos < end && *pos == '=')
  ...
  case '?': return Token(TokenType::QuestionMark, token_begin, ++pos);
  case '^': return Token(TokenType::Caret, token_begin, ++pos);
  case ':': {
    ++pos;
    if (pos < end && *pos == ':')
  ...
  case '\xE2': {
    ...
    if (pos + 5 < end && pos[0] == '\xE2' && pos[1] == '\x80' &&
        (pos[2] == '\x98' || pos[2] == '\x9C')) {
      const char expected_end_byte = pos[2] + 1;
      TokenType success_token = pos[2] == '\x98' ? TokenType::StringLiteral
                                                 : TokenType::QuotedIdentifier;
      TokenType error_token = pos[2] == '\x98'
                                  ? TokenType::ErrorSingleQuoteIsNotClosed
                                  : TokenType::ErrorDoubleQuoteIsNotClosed;
      pos += 3;
      return quotedStringWithUnicodeQuotes(
          pos, token_begin, end, expected_end_byte, success_token, error_token);
    }
  ...
  default:
    if (*pos == '$') {
      ...
      if (heredoc_name_end_position != std::string::npos) {
        ...
        std::string_view heredoc = {token_stream.data(),
                                    heredoc_size}; // NOLINT
    ...
    /// We will also skip unicode whitespaces in UTF-8 to support for queries
    /// copy-pasted from MS Word and similar.
...
const char *getErrorTokenDescription(TokenType type) {
  switch (type) {
  case TokenType::Error: return "Unrecognized token";
  ...
  case TokenType::ErrorWrongNumber: return "Wrong number";
  case TokenType::ErrorMaxQuerySizeExceeded: return "Max query size exceeded";
